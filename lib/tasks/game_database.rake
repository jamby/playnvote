...
namespace :playnvote do
  ...
  task :create_games_database => :environment do
    ...
    PLATFORMS.each do |plat|
      ...
      puts "Platform checking: #{plat}, Max Results: #{max_results}"
      offset = 0

      while offset <= max_results do
        result['results'].each_index do |i|
          unless games['games'].find{|g| g["id"] == result['results'][i]['id']}
            puts "Game looking up: #{result['results'][i]['id']}, current: #{(offset+i)}"
            ...
            games['games'] << { 'id' => result['results'][i]['id'],
                                ...
                                'developer' => viewed_game['results']['developers'] ? Array.new(viewed_game['results']['developers'].length){ |dev| viewed_game['results']['developers'][dev]['name'] }.compact : ["No Data for Developer"],
                                'publisher' => viewed_game['results']['publishers'] ? Array.new(viewed_game['results']['publishers'].length){ |pub| viewed_game['results']['publishers'][pub]['name'] }.compact : ["No Data for Publisher"],
                                'genre' => viewed_game['results']['genres'] ? Array.new(viewed_game['results']['genres'].length){ |gen| viewed_game['results']['genres'][gen]['name'] }.compact : ["No Data for Genre"],
    ...
    games['games'].each do |game|
      unless Game.find_by_giant_bomb_id(game['id'])
        unless game['platforms'].blank?
          Game.create(title: game['title'], release_date: game['release_date'], cover_art: game['cover_art'], platform: game['platforms'], giant_bomb_id: game['id'], developer: game['developer'], publisher: game['publisher'], genre: game['genre'])
        end
      end
    end
  end
